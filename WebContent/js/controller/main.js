var app = angular.module('mainApp', ['ui.bootstrap', 'ui.sortable', 'AxelSoft', 'ngRoute', 'ngTagsInput', 'ngStorage'])
	...
	.controller('MainController', function($scope, $sessionStorage, $http, $location, $routeParams, $anchorScroll, $timeout, $rootScope) {
		...
		$scope.siape = siape;
		$scope.homeSiape = siape;
		$sessionStorage.siape = siape;
...
//				console.log("$scope.list", $scope.list);
			...
			$http.get(url + "rest/disciplina/listaDisciplinaTurmaProfessorPrioridade/"+$scope.siape+'/'+$scope.semestreSelecionado.ano+'/'+$scope.semestreSelecionado.semestre).success(function(response) {
				tmpList =  $scope.listPrioridade = response; 
//				console.log("$scope.list", $scope.list);
			});

			
			$http.get(url + "rest/disciplina/listaDisciplinaTurmaProfessorNotIn/"+$scope.siape+'/'+$scope.semestreSelecionado.ano+'/'+$scope.semestreSelecionado.semestre).success(function(response) {
				$scope.listNotIn = response;
//				console.log($scope.listNotIn[0]);
			});
			...
			$http.get(url + 'rest/professor/buscaPorSemestre/'+$scope.semestreSelecionado.ano+'/'+$scope.semestreSelecionado.semestre).success(function(response) {
			  	if(response && response.length > 0){
				  	$scope.professoresDoSemestre = response;
			  	}
	//		  	console.log($scope.professores[0]);
		  	});
		...
		$scope.submeterFilaPrioridades = function() {
			...
			angular.forEach($scope.listPrioridade, function(value, key){
		...
		$scope.loadItems = function(query){
			...
			for ( var i in filtred) {
				...
				if($scope.disciplinasTags.indexOf(nome)== -1){
					$scope.disciplinasTags.push(nome);
				}
			}
		...
		$scope.loadItems2 = function(query){
			$scope.disciplinasTags = [];
			var list = angular.copy($scope.listNotIn);
			if(list.length <= 0){
				return;
			}
		...
		$scope.loadItems3 = function(query) {
			$scope.professoresTags = [];
			var professor = angular.copy($scope.professoresDoSemestre);
			if(professor.length <= 0) {
				return;
			}

			var filtred = professor.filter(function(prof){
				return prof.nome.toLowerCase().match(query.toLowerCase());
			});

			for(var i in filtred) {
				var nome = filtred[i].nome.trim()+" - "+filtred[i].siape.trim();
				if($scope.professoresTags.indexOf(nome)== -1){
					$scope.professoresTags.push(nome);
				}
			}
			return $scope.professoresTags;
		}

		$scope.marcaHorario2 = function(tag,adiciona,professor) {
			
			$scope.siapeSelecionado = [];
			
			if(professor == true) {

				$scope.siapeSelecionado = tag.text.split("-");

				$http.get(url + "rest/disciplina/listaDisciplinaTurmaProfessorMarcaHorario/"+$scope.siapeSelecionado[1]+'/'+$scope.semestreSelecionado.ano+'/'+$scope.semestreSelecionado.semestre).success(function(response) {
					for(var i = 0; i < response.length; i++) {

						var disciplinaSelecionada = response.filter(function(e){
							return (e.codigoDisciplina.trim().toLowerCase() == response[i].codigoDisciplina.trim().toLowerCase()) && (e.turma.trim() == response[i].turma.trim());
						});

						var d = disciplinaSelecionada[0];
						var horarios = d.horarios;

						if (adiciona) {
							for(var j = 0; j < horarios.length; j++) {
								var horarioTabela = document.getElementsByClassName(horarios[j].dia+""+horarios[j].horario);
								horarioTabela[0].textContent = horarioTabela[0].textContent.replace(d.codigoDisciplina.trim()+'('+d.turma.trim()+") - ",d.codigoDisciplina.trim()+'('+d.turma.trim()+") (2x) - ");

								if(horarioTabela[0].textContent.indexOf(d.codigoDisciplina.trim()+'('+d.turma.trim()+") (2x)") === -1) {
									horarioTabela[0].textContent = horarioTabela[0].textContent+d.codigoDisciplina.trim()+'('+d.turma.trim()+") - ";
								}
							...
							for(var j = 0; j < horarios.length; j++) {
								...
								if(horarioTabela[0].textContent.indexOf(d.codigoDisciplina.trim()+'('+d.turma.trim()+") (2x)") !== -1) {
									horarioTabela[0].textContent = horarioTabela[0].textContent.replace(d.codigoDisciplina.trim()+'('+d.turma.trim()+") (2x) - ",d.codigoDisciplina.trim()+'('+d.turma.trim()+") - ");
								} else {
									horarioTabela[0].textContent = horarioTabela[0].textContent.replace(d.codigoDisciplina.trim()+'('+d.turma.trim()+") - ","");
								}
				...
				var codDisciplina = tag.text.split("-");
				
//				console.log("codDisciplina", codDisciplina);
				
				var auxCodTurma = '';
				if(codDisciplina && codDisciplina[1]){
					auxCodTurma = codDisciplina[1].split('(');
					auxCodTurma = auxCodTurma[1].replace(")","");
				}
				
				var codTurma = '';
				if(auxCodTurma!=''){
					codTurma = auxCodTurma;
				}
				
//				console.log("codTurma", codTurma);
//				return;
				
				var disciplinaSelecionada = $scope.list.filter(function(e){
//					console.log(e.codigoDisciplina.trim().toLowerCase()==codDisciplina[0].trim().toLowerCase());
//					console.log(e.codigoDisciplina.trim().toLowerCase(),' - ', codDisciplina[0].trim().toLowerCase());
//					console.log(e.turma.trim(),' - ', codTurma.trim());
					return (e.codigoDisciplina.trim().toLowerCase() == codDisciplina[0].trim().toLowerCase()) && (e.turma.trim() == codTurma.trim());
				});
				
				if(disciplinaSelecionada && disciplinaSelecionada.length<=0){
					disciplinaSelecionada = $scope.listNotIn.filter(function(e){
						return (e.codigoDisciplina.trim().toLowerCase() == codDisciplina[0].trim().toLowerCase()) && (e.turma.trim() == codTurma.trim());
					});
				}
				
//				console.log("disciplinaSelecionada", disciplinaSelecionada[0]);
//				return;
		...
		$scope.tagInput = {}
		$scope.tagInput.tags = [];
		$scope.limpaTabelaHorarios = function(){
			$scope.tagInput.tags = [];
			$scope.tagInput.tagsOutras = [];
			$scope.tagInput.tagsProfessores = [];
			$('#tabelaHorarioConflito tr').each(function(){
			    $(this).find('td').each(function(){
			        if($(this)[0].bgColor && $(this)[0].bgColor != "#FFFFFF"){
			        	$(this)[0].bgColor = "#FFFFFF";
			        	$(this)[0].textContent = "";
			        }
			    })
			})
