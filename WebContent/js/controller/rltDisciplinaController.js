app.controller('RltDisciplinaController',['utils','$sessionStorage', '$scope', '$http', '$location', '$routeParams', '$anchorScroll', '$timeout', '$modal',
  ...
  $http.get(url + 'semestre/listar').success(function(response) {
	  ...
	  $http.get(url + 'rltProfessor/listarDisciplina/'+$scope.semestreSelecionado.ano+'/'+$scope.semestreSelecionado.semestre).success(function(response) {
		  if(response && response.length > 0){
			  var nomeAtivo = response[0].nomeDisc;
			  var nomeClassAtiva = 'relatClass1';
			  $scope.professores = response.map(function(e){
				  if(nomeAtivo != e.nomeDisc){
					  nomeAtivo = e.nomeDisc;
