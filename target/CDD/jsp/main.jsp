		...
		<accordion close-others="false">
			...
			<accordion-group ng-show="etapaAtiva('escolherPrioridade')" heading="Escolher prioridade de disciplinas" is-open="false">
				...
				<div class="row">
					<!--
					<div class="col-md-8">
						<select name="repeatSelect" ng-model="disciplinaAdd" class="form-control">
	     						<option ng-repeat="d in disciplinas" value="{{d}}">{{d.nomeDisciplina}}</option>
	   					</select>
						<br>selected: {{disciplinaAdd}}
   					</div>
   					<div class="col-md-2">
   						<button type="button" class="btn btn-primary" ng-click="adicionarDisciplina(disciplinaAdd)">ADICIONAR</button>
   					</div>
   					-->
	    	...
	    	<accordion-group ng-show="etapaAtiva('verificarHorarios')" heading="Verificar horários das disciplinas" is-open="false">
	    		<div class="col-md-12">
	    			<div class="row">
						<div class="col-md-4" ng-repeat="d in list" style="height: 60px;">
							<input value="{{d}}" type="checkbox" ng-model="horarioDisciplinaCheckbox" ng-click="marcaHorario($event, d)">
							<span style="font-size: 9px;">{{d.codigoDisciplina}} - {{d.nomeDisciplina}} ({{d.turma}}) - {{d.codigoCurso}}</span>
						</div>
	      			</div>
